...
joint3(i,j)=rad2deg(acos((L2^2+L3^2-L1^2)/(2*L2*L3)));
...
joint3_1(i,j)=rad2deg(acos((L2^2+L3_1^2-L1^2)/(2*L2*L3_1)));
end
end
%%
...
%calibrition sensor
sw=0;
CA=strcat(num2str(180-joint0(i,j)),',',num2str(joint1(i,j)),',',num2str(joint2(i,j)),',',num2str(joint3(i,j)),',',num2str(dt),',',num2str(sw));
fprintf(x,CA);
y=fscanf(x);
if y==2
    disp("Scan finished");
